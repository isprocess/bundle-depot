...
WORKFLOW = ROOT / ".github" / "workflows" / "p398245.yml"
...
class P398245ContractTest(unittest.TestCase):
    def setUp(self):
        self.text = WORKFLOW.read_text(encoding="utf-8")
        self.jobs = extract_jobs(self.text)

    def test_event_permissions_concurrency(self):
        self.assertIn("types: [build_398245]", self.text)
        self.assertRegex(self.text, r"(?m)^permissions: \{\}\s*$")
        self.assertRegex(self.text, r"(?m)^  group: p398245\s*$")
    ...
    def test_build_verifies_fetches_and_parses_lock(self):
        ...
        self.assertIn("check-bootstrap-window.py", build)
    ...
    def test_publish_secret_mapping(self):
        ...
        self.assertIn("project: ${{ secrets.SOURCE_GIT_PROJECT_398245 }}", publish)
